...
export async function GET(request: Request) {
  ...
  const limit = parsePositiveInt(params.get("limit"), 500, 5000);
  const window = analyticsWindowFromRequestUrlOnly(request);
  return jsonResult(await windowedResult(
    window,
    getFitness(window.primary).then((result) => result.status === "ok"
      ? withTruncation(result, limit) : result),
    window.comparison
      ? getFitness(window.comparison).then((result) => result.status === "ok"
        ? withTruncation(result, limit) : result)
      : null,
  ));
}
